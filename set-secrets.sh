...
SECRET_NAMES=(
  ...
  "AI_GATEWAY_API_KEY:Vercel AI Gateway API key for the frontier cascade (optional, press Enter to skip)"
  "ANTHROPIC_API_KEY:Anthropic (Claude) API key for direct frontier fallback (optional, press Enter to skip)"
  "OPENAI_API_KEY:OpenAI API key for direct frontier fallback (optional, press Enter to skip)"
...
if [ "${1:-}" = "--list" ]; then
  ...
  for org in "${ORGS[@]}"; do
    echo -n "$org: "
    gh secret list --org "$org" 2>&1 | grep -E 'OPENROUTER|GEMINI|GROQ|CONTEXT7|AI_GATEWAY|ANTHROPIC|OPENAI' | tr '\n' ' ' || echo "(none or no access)"
    echo
  done
  echo
  ...
  while IFS= read -r repo || [ -n "$repo" ]; do
    ...
    echo -n "$repo: "
    gh secret list -R "$repo" 2>&1 | grep -E 'OPENROUTER|GEMINI|GROQ|CONTEXT7|AI_GATEWAY|ANTHROPIC|OPENAI' | tr '\n' ' ' || echo "(none)"
    echo
  done < "$FLEET_FILE"
  echo
  echo -n "$GATEWAY_REPO: "
  gh secret list -R "$GATEWAY_REPO" 2>&1 | grep -E 'OPENROUTER|GEMINI|GROQ|CONTEXT7|AI_GATEWAY|ANTHROPIC|OPENAI' | tr '\n' ' ' || echo "(none)"
  echo
  exit 0
fi
