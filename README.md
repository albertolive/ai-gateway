...
**CI minutes are a cost too.** The cascade is bounded by a wall-clock budget, `budget_s` (default 300s, override with `AI_GATEWAY_BUDGET_S`). Without it the worst case was open-ended: 7 providers x 2 attempts x a 120s socket timeout is ~28 minutes on paper, and because `urlopen`'s timeout applies per socket read rather than to total elapsed time, one slow responder stretched a real run to **44 minutes** on 2026-07-29. On a private repo that is billable time spent discovering everything is down.

Two changes bound it. The budget stops the cascade and reports which providers it never reached, and a 429 whose body names a DAILY allowance is no longer retried: that quota resets tomorrow, not in five seconds, so the retry was a guaranteed-useless extra round trip per dead provider. A per-MINUTE limit still gets its retry, because there five seconds genuinely helps. Measured on a fully-capped cascade: **1.8s instead of ~38s**. The tail exists because the all-free cascade genuinely ran out on 2026-07-28 (Gemini RPD cap, Groq 403, OpenRouter free-models-per-day) and left a PR with no review at all — and PR review is low-volume, so the worst case is a few cents on the days the free tier is exhausted.
