...
**CI minutes are a cost too.** The cascade is bounded by a wall-clock budget, `budget_s` (default 900s, override with `AI_GATEWAY_BUDGET_S`). Without it the worst case was open-ended: 7 providers x 2 attempts x a 120s socket timeout is ~28 minutes on paper, and because `urlopen`'s timeout applies per socket read rather than to total elapsed time, one slow responder stretched a real run to **44 minutes** on 2026-07-29. On a private repo that is billable time spent discovering everything is down.
